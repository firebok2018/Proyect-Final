...
public class FrmVenta extends JInternalFrame implements ActionListener {
	...
	public FrmVenta() {
		...
		lblDni = new JLabel("Cod Cli.:");
		lblDni.setBounds(194, 22, 46, 14);
		...
		cboTipo.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (x==1) {
					conVen.SubtiposM(cboSubTipo,1);
				}
				if(x==2){
					conVen.SubtiposB(cboSubTipo,2);
				}
				if(x==3){
					conVen.SubtiposE(cboSubTipo,3);
				}
		...
		userV=ConexionSign_In.usuarioIngre;
	...
	int tipo(){return cboTipo.getSelectedIndex();}
														...
														try {
															...
															if(nc<1){
																...
																try{
																	...
																	String usr=userV;
																	if(usr.length()!=0){
																		Venta x= new Venta(CodVenta(), codCli(), n+a, t, p, nc, usr);
																		conVen.add_venta(x);
																		listar();
																	}else{
																		this.dispose();
																		Alerte xd=new Alerte();
																		xd.setVisible(true);
	...
	void eliminarVenta(){
		System.out.println("Eliminado");
				//String xd="WISDS";
				Venta xy= new Venta(CodVenta());
				conVen.delete_venta(CodVenta());
				listar();
	...
	void Search_Cliente(String x){
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "El Codigo del cliente "+x+" No se encuentra registrado!");
		}finally {
			try {
				con.close();
			} catch (SQLException e) {
