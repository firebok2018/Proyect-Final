...
public class FrmRegistroClientes extends JInternalFrame implements ActionListener {
	...
	public FrmRegistroClientes() {
		...
		lblApellido.setBounds(26, 98, 68, 14);
		...
		lblTelefono.setBounds(26, 136, 68, 14);
		...
		lblMesa.setBounds(219, 136, 46, 14);
		...
		cboEstado.setBounds(472, 133, 91, 20);
		...
		lblEstado.setBounds(426, 136, 46, 14);
		...
		btnBuscar.setBounds(447, 15, 116, 33);
					...
					if (s.length()<4) {
						...
						try {
							...
							if (f.length()<9||f.length()>9) {
								...
								Cliente nCli = new Cliente(codigo(), dni, n, s, t, mesa(),estado());								
	...
	protected void actionPerformedBtnBuscar(ActionEvent arg0) {
		Search_Cliente(codigo());
	}
	void Search_Cliente(String x){
		Connection con= null;
		CallableStatement sp= null;
		ResultSet rs=null;
		try {
			con=new Conexion().getConexion();
			sp=con.prepareCall("{ call search_cliente(?)}");
			sp.setString(1, x);
			rs=sp.executeQuery();
			while (rs.next()) {
				txtCodigoCliente.setText(rs.getString(1));
				txtDniCliente.setText(rs.getInt(2)+"");
				txtNombreCliente.setText(rs.getString(3));
				txtApellidoCliente.setText(rs.getString(4));
				txtTelefono.setText(rs.getInt(5)+"");
				cboNumMesa.setSelectedIndex(rs.getInt(6));
				cboEstado.setSelectedIndex(rs.getInt(7));	
			}
		} catch (SQLException e) {
			System.out.println(e.getSQLState()+e.getErrorCode()+e.getMessage()+e.getLocalizedMessage());
			JOptionPane.showMessageDialog(null, "El Codigo "+x+" No se encuentra registrado!");
		}finally {
			try {
				con.close();
			} catch (SQLException e) {
